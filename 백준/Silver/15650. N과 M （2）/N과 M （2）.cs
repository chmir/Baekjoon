 ...
 * 15649번과 달리 오름차순으로 출력해야하므로... dfs에 반복문 부분을 살짝 수정했다
 * 하지만 dp는 아무리봐도 다른문제보다 확실하게 이해가 안되기때문에, 복습해야할 거 같다.
*/
...
class Program
{
    ...
    static void dfs(int index, int cnt, StreamWriter sw)
    {
        ...
        {
            //iter변수가 index값부터 시작하여 N까지 증가하기에, 
            //이미 선택된 숫자보다 작은 숫자가 나오지 않으므로, 
            //결과적으로 수열은 오름차순을 유지할 것이다
